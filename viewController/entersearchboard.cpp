#include "entersearchboard.h"
#include <QPainter>
#include <QMouseEvent>
#include "assert.h"
#include <QDebug>
EnterSearchBoard::EnterSearchBoard(ColorStyle *style, chess::Board *board, QWidget *parent) :
    Chessboard(parent)
{
...
void EnterSearchBoard::paintEvent(QPaintEvent *e) {
...
void EnterSearchBoard::resizeEvent(QResizeEvent *) {
    this->setMinimumWidth(this->height()*1.35);
}

void EnterSearchBoard::mousePressEvent(QMouseEvent *m) {
    int x = m->x();
    int y = m->y();
...
bool EnterSearchBoard::clickedOnBoard(int x, int y) {
    int boardSize = 0;
    int squareSize = 0;
...
bool EnterSearchBoard::clickedOnPiceceSelector(int x, int y) {
    int boardSize = 0;
    int squareSize = 0;
    this->calculateBoardSize(&boardSize, &squareSize);
...
uint8_t EnterSearchBoard::getSelectedPiece(int x, int y) {
    int boardSize = 0;
    int squareSize = 0;
    this->calculateBoardSize(&boardSize, &squareSize);
...
// and y is y-axis
QPoint EnterSearchBoard::getBoardPosition(int x, int y) {
    int boardSize = 0;
    int squareSize = 0;
    this->calculateBoardSize(&boardSize, &squareSize);
...
/*
void EnterSearchBoard::calculateBoardSize(int *boardSize, int *squareSize) {

    QSize size = this->size();
    qDebug() << "EnterSearchBoard Size:  " << size;
...
void EnterSearchBoard::setToInitialPosition() {
    delete this->board;
    this->board = new chess::Board(true);
    this->update();
}

void EnterSearchBoard::setToCurrentBoard() {
    delete this->board;
    this->board = new chess::Board(this->currentGameBoard);
    this->update();
}

void EnterSearchBoard::clearBoard() {
    delete this->board;
    this->board = new chess::Board();
    this->update();
}

chess::Board* EnterSearchBoard::getCurrentBoard() {
    return this->board;
}

void EnterSearchBoard::drawBoard(QPaintEvent *event, QPainter *painter) {
    ...
    for(int i=0;i<7;i++) {
        for(int j=0;j<2;j++) {
            ...
            qDebug() << "piece type here: " << piece_type;
